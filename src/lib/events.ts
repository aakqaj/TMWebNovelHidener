...
import { clearContents, renderContent, renderPre, renderNext, toNext, toPre, reloadConfigPage } from "./page";
import { header, headerMask } from "./template";
import { isNumeric, debounce, keyListener, updateStyle } from "./utils";
...
export function addScrollEvent() {
    ...
    novelDiv.addEventListener('scroll', debounce(() => {
...
export function addGlobalHotKeyListener() {
    ...
    keyListener(document.body, config.config.HotKey.hiden, () => {
        ...
        if (config.config.hidenMode === "hiden") {
